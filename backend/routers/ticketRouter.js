...
router.post("/get_price", async (req, res) => {
   ...
   try{
          ...
          const rec = await Ticket.findOne({name : ticket_name})
              return   res.send({result : 'success',data : rec.price,id : rec._id})        
...
router.post("/ticket_price", async (req, res) => {
     const {address,price} = req.body
     try{
        const result =  await Ticket.findOne({creator : address})
        result.price = price
   ...
   try{
         ...
         if(user){
           ...
           if(user.role == 2){
               ...
               const employeer_rec = await User.findById(user.employeer)

               const client = await User.findOne({algo_address : user_address})
        
              if(!client) 
                   res.send({result : 'failed', msg:'Client address is not exist'})
              const rec = await Ticket.findById(type) 
              console.log(rec.creator,employeer_rec.algo_address)
              if(rec.creator != employeer_rec.algo_address)
                    return res.send({result : 'failed', msg : 'Not Allowed for the other business ticket.'})
              
              if(!rec) 
                   res.send({result : 'failed', msg : 'User address is not exist'}) 
              ...
              saveHistory(user_address,address,user.employeer,type,1)
               ...
               res.send({result : 'failed', msg : 'Not Allowed the role'})
           }
         }
         else
          res.send({result : 'failed'})
