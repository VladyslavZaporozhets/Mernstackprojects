...
router.post("/setPassword", async (req, res) => {
    let { address, oldPassword, newPassword, confirmPassword } = req.body;
    console.log(address, "address")

    if(!address || !newPassword || !oldPassword || !confirmPassword || newPassword != confirmPassword) return res.status(401).json({ msg: "Invalid Reset Password" });

    const saltRounds = 10;
    const passwordHash = await bcrypt.hash(oldPassword, saltRounds);
    ...
    if (user){
        ...
        if(!matchPassword){
            return res.status(401).json({ msg: "Old Password is invalid!" });
        }
        const newpasswordHash = await bcrypt.hash(newPassword, saltRounds);
        User.findByIdAndUpdate(user._id, { passwordHash: newpasswordHash, emailConfirmed: true })
                .then(user => {
                    ...
                    return res.status(401).json({ msg: "New Password Save Error!" });
                })
    } else {
        return res.status(401).json({ msg: "The Account is not exist" });
    }
});

router.post("/setEmail", async (req, res) => {
    let { address, oldEmail, newEmail} = req.body;
    console.log(address, "address")

    if(!address || !oldEmail || !newEmail || oldEmail == newEmail) return res.status(401).json({ msg: "Invalid Reset Email" });

    const user = await User.findOne({algo_address : address, email : oldEmail});
    // console.log(user._id, "userid");
    if (user){
        return res.json({ user: user });
    } else {
        return res.status(401).json({ msg: "The Account is not exist with email" });
    }
});
                ...
                if(type == 'email')
                    {
                        ...
                        var mailOptions = {
                          from: 'veniaminit9@gmail.com',
                          to: email,
                          subject: 'Block Reward SignUp Verify Code',
...
router.post("/verifycodeforEmail",async (req,res)=>{
    const {code, oldEmail, newEmail} = req.body;
    if(!code || !oldEmail || !newEmail) {
        return res.status(400).json({ msg: "Verify Code is not correct!" });
    }
    if(verifycodeList[newEmail] == '' ){
        return res.status(400).json({ msg: "Verify Code is not correct!" });
    } else {
        if (verifycodeList[newEmail] == code) {
            const existingUser = await User.findOne({ oldEmail });
            if (!existingUser) {
                return res.status(400).json({ msg: "User is not exist!" });
            }
            const result = await User.findByIdAndUpdate(existingUser._id, {email : newEmail, emailConfirmed: true});
            const token = jwt.sign(
                {
...
//------------------------ Generate Algorand account---------------------
        ...
        try
            {
                console.log("new");
                await transfer(manager.addr,algo_address,manager.sk, (process.env.REACT_APP_ALGO_PRESENT)* 1000000)
...
router.post("/create_arc_19",async(req,res)=>{
 ...
 const {address,name, unit_name , description, url_p,reserveAddress_p,url_v,reserveAddress_v,algo,eth,matic,usdc,level,bonus_amount} = req.body;    
 if(!address) 
   ...
   return  res.send({ result: "failed"})
    ...
    try{
        await CreateArc19(address,address,name,unit_name,description,url_p,reserveAddress_p,getSecrectKey(algo_sk))    
         ...
         const listed = '0'      
         const bonus = bonus_amount?bonus_amount:0
...
router.post("/config_arc_19",async(req,res)=>{
 ...
 const {address,id,name, unit_name , description, url_p,reserveAddress_p, url_v,reserveAddress_v,algo,eth,matic,usdc,bonus_amount} = req.body;    
    
    if(!address) 
        return    res.send({ result: "failed"})

 const membership_d = await Membership.findOne({type:name})
    if(!membership_d) 
        return    res.send({ result: "failed"})
    ...
    const algo_sk = user.algo_sk;
   try{
        await ConfigArc19(address,id,name,unit_name,description,url_p,reserveAddress_p,getSecrectKey(algo_sk))   
        membership_d.usdc = usdc
        membership_d.algo = algo
        membership_d.eth = eth
        membership_d.matic = matic
        membership_d.picture = url_p
        membership_d.video = url_v
        membership_d.bonus = bonus_amount?bonus_amount:0;
...
router.post("/update_TicketSellRole",async(req,res)=>{

    try{
 
           const {address,id,TicketSellRole} = req.body;
           if(!address) 
             return    res.send({result : 'failed',msg : 'Not Exist User'})
           ...
           const user = await User.findOne({algo_address : address});
             ...
             if(!client) {
              return    res.send({result : 'failed',msg : 'Not Exist User'})
                 ...
                 client.TicketSellRole = TicketSellRole 
                 await client.save()
                 ...
                 var mailOptions = {
                   from: 'veniaminit9@gmail.com',
                   to: client.email,
                   subject: 'Reset Ticket Sell Role',
                   html: `<html>
                              Your Ticket Sell Role is `+ TicketSellRole == '1'? 'allowed' : 'disabled' +`!.<br>
...
router.post("/get_user_TicketSellRole", async(req, res) => {
    ...
    try{
            ...
            var user = await User.findOne({algo_address : address})
            if(!user || user.TicketSellRole == null) {
                console.log(user,1)
                return res.send({result : 'failed'})    
            }
            if (user.isBusinessOwner == '0' && user.employeer != ''){
                user = await User.findById(user.employeer)
            }
            if(!user || user.TicketSellRole == null) {
                return res.send({result : 'failed'})    
                console.log(user,2)
                ...
                res.send({result : 'success', data : user.TicketSellRole})
            }
...
router.post("/get_business_list",async(req,res)=>{
  ...
  var sender = await User.findOne({algo_address : address})
  var users;
  if (sender.role > 0) {
        users = await  User.find({isBusinessOwner : '1', role:'1' })
  } else
        users = await  User.find({isBusinessOwner : '1'})
  var result = []
  try{
    for(var user of users){
        if(user.role == 0) continue;
        // if(user.company  != '') {
            result.push({id:user._id,owner : (user.firstName + user.lastName),company:user.company,vat : user.vat,address : user.address, role : user.role , path : user.algo_address, TicketSellRole : user.TicketSellRole })
        // }
...
router.post("/get_address_list",async(req,res)=>{
  ...
  const users = await  User.find({},{firstName:1, lastName:1, email:1, algo_address : 1, eth_address : 1, role : 1})
  var result = []
...
router.post('/transfer_matic',async(req,res)=>{
 ...
 export const GetBonusbyBRT = async (address,sender, amount)=>{
    const admin = await User.findOne({algo_address : sender})
    if(admin){
        console.log("sending bonus...")
        await transferToken(sender,address,amount,getSecrectKey(admin.algo_sk))
    }else{
