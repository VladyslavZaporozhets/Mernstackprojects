...
export const Employee = () => {
  ...
  const onSearch = (val)=>{
    setSearchString(val)
    var data = srcData.filter(ele => (ele.firstName != '' &&  ele.firstName.toLowerCase().indexOf(val.toLowerCase()) >= 0 || ele.lastName != '' &&  ele.lastName.indexOf(val) >= 0 || ele.phoneNumber != '' &&  ele.phoneNumber.indexOf(val) >= 0 ||  ele.email.indexOf(val) >= 0));  
    setTempData(data)   
