...
export const WithDraw = (props) => {
  ...
  const onWithdraw = async()=>{
    ...
    if(amount_value == 0 || isNaN(amount_value)){
      toast.error("Please enter the amount correctly")    
      return;
    }
    else if (wallet.name == 'ALGO' && amount_value > (wallet.price - process.env.REACT_APP_ALGO_PRESENT)){
      toast.error("You should leave at least " + process.env.REACT_APP_ALGO_PRESENT + ' Algo.')
      return;
    }
    if(amount_value > wallet.price){
      toast.error("Please enter the amount correctly")    
      return;
    }
