...
export const TicketHistory = (props) => {
  ...
  const onSearch = (val)=>{
    setSearchString(val)
    var data = srcData.filter(ele => (ele.username != '' &&  ele.username.toLowerCase().indexOf(val.toLowerCase()) >= 0 || ele.employee != '' &&  ele.employee.toLowerCase().indexOf(val.toLowerCase()) >= 0 || ele.business != '' &&  ele.business.indexOf(val) >= 0));  
    setTempData(data)   
  }
    ...
    return (
        ...
        <Container>
             <DashboardHeading title = 'Ticket History' responsive = {false}>
