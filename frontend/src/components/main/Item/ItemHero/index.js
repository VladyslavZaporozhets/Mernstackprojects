...
export const ItemHero = (props) => {
  ...
  const startMint = async ()=>{
    ...
    try{
      var url = ''
      if(currentProcess == 'buy'){
        url = 'membership/buy_card';
      }else if (currentProcess == 'upgrade'){
  ...
  const LoadNFTs  = async(asset_id)=>{
   ...
   const getBalance = async()=>{
    if(user_address){
      ...
      var price_list = []
      price_list[0] = (clientInfo['amount']) / 1000000;// Algo balance
