  ...
  if(user_address){
    try{
      ...
      for (var asset of assets) {
        ...
        if(asset_map.name == undefined) continue;
 ...
 useEffect(() => {
  ...
  const LoadNFTs  = async(asset_id)=>{
    try{
      ...
      if(reserveURL == undefined) return [];
...
/*****************Document Part*********************/  
  return (  <>
    ...
    <div style = {{padding:20,marginTop:20,marginLeft:30}}>
      ...
      role < 2 && (
          <>
              <DashboardHeading title = {role == 0 ? 'Platform NFT' : 'Company Membership'} responsive = {false}>
                <ButtonWrapper>            
        ...
        {role == 3 && <>
          <DashboardHeading title = '' responsive = {false}/>
    ...
    <Modal width='370px' open={isMint} onClose={() => setIsMint(false)}>
