...
import EnHome from '@meronex/icons/en/EnHome';
...
export const Header = (props) => {
 ...
 useEffect(()=>{
  ...
  const resourceSubMenuList = [
    { path: '/about', title: 'About BlockReward'},
 ...
 var dashboardMenuList = [
   { path: '/u/wallets', title: 'Account Info' },
   { path: '/u/MintMembership', title: title},
 ]
 ...
 if (role == 1){
  dashboardMenuList = [
   ...
   if(isallowedSellTicket){
    dashboardMenuList.push({ path: '/u/ticket', title: 'Ticket Mint' })
   }
 }
 else if (role == 3){
  dashboardMenuList = [
     { path: '/u/wallets', title: 'Account Info'  },
     { path: '/u/offers', title: 'Redeemed Discounts' },
     { path: '/u/ticket', title: 'POAP' },
     { path: '/u/MintMembership', title: title },
   ]
 }
  ...
  return (
    <>
      <Container noShadow={pathname.includes('/u/') || pathname.includes('/c/')}>
        <InnerContainer>
          <LeftWrapper>
            <img src={theme?.isLightMode ? themeOriginal.images.logoDark : themeOriginal.images.logo} alt='logo' onClick={() => {localStorage.setItem('locked','1');navigate('/')}} />
          ...
          <RightWrapper>
            ...
            {width < 1200 && (
             ...
             {
              <IconButton onClick={() => navigate('/u/dashboard', { state: { active: true } })}>
              <EnHome />
              </IconButton>
             } 
      ...
      <MobileMenuWrapper style={{ width: isMobileMenu && '75%' }}>
        <div>
          ...
          <LogoWrapper>
            ...
            >
              <MobileDropDown>
                {dashboardMenuList.map((subMenu, i) => (
                  <div key={i} onClick={() => handleClickMenu(subMenu.path, subMenu.params)}>{subMenu?.title}</div>
                ))}
              </MobileDropDown>
            </Accordion>  
