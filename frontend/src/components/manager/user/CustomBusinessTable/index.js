...
import { TicketRoleBtn } from '../../../shared/TicketRoleBtn' 
...
//   const {rowsofPage} = props
  ...
  const cols = isMobile ? ['Company','VAT Number','Owner','Address', 'Ticket Sell Role', 'Status']:['Company','VAT Number','Status']
          ...
          {tempData.map((item, i) => (
            ...
            {isMobile &&  <td>{item.owner}</td> }  
             {isMobile && <td>{item.address}</td> } 
              <td><TicketRoleBtn allowed = {item.TicketSellRole == '1'} id = {item.id} TicketSellRole = "1" /></td>
